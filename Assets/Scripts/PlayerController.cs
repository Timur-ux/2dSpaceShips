...
public class PlayerController : MonoBehaviour {
	...
	void Start() {
		...
		attack_ = GetComponentInChildren<PlayerAttack>();
		attack_.enabled = false;
	}
	...
	void stateUpdateIfNeeded() {
		...
		if (hitData.collider != null && hitData.distance < 0.2f) {
			...
			collider_.offset = new Vector2(collider_.offset.x, crouch ? crouchColliderOffset : standColliderOffset);
		} else {
			state_ = State.InSpace;
			if (!crouch)
				collider_.offset = new Vector2(collider_.offset.x, jumpColliderOffset);
	...
	public void OnTriggerEnter2D_(Collider2D other) {
		...
		if (other.TryGetComponent<Rigidbody2D>(out Rigidbody2D rb))
			rb.AddForce(lookDirection_ * attackForce);
	}
}
